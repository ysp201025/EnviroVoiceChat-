...
class UIManager {
  constructor() {
    this.elements = {
      ...
      // NEW: Screen 2 - Profile section
      profileSection: document.getElementById("profileSection"),
      saveProfileBtn: document.getElementById("saveProfileBtn"),
      editProfileBtn: document.getElementById("editProfileBtn"),
      profileGamertag: document.getElementById("profileGamertag"),
  ...
  updateGamertagStatus(gamertag) {
    ...
    this.elements.gamertagStatus.style.color = gamertag ? `#3c8527` : "#ef4444";
  }
  ...
  // Show screen: "setup" | "profile" | "call"
  showScreen(screen) {
    this.elements.setupSection.style.display   = screen === "setup"   ? "block" : "none";
    this.elements.profileSection.style.display = screen === "profile" ? "block" : "none";
    this.elements.callControls.style.display   = screen === "call"    ? "flex"  : "none";
  }

  // Legacy shim used elsewhere in codebase
  showCallControls(show) {
    if (show) {
      this.showScreen("call");
    } else {
      const saved = UIManager.loadProfile();
      this.showScreen(saved ? "profile" : "setup");
    }
  }

  // Show profile values on Screen 2
  renderProfile(gamertag, url) {
    this.elements.profileGamertag.textContent = gamertag;
    this.elements.profileUrl.textContent = url;
  }

  // --- Static localStorage helpers ---
  static saveProfile(gamertag, url) {
    localStorage.setItem("envirovoice_gamertag", gamertag);
    localStorage.setItem("envirovoice_url", url);
  }

  static loadProfile() {
    const gamertag = localStorage.getItem("envirovoice_gamertag");
    const url = localStorage.getItem("envirovoice_url");
    if (gamertag && url) return { gamertag, url };
    return null;
  }

  static clearProfile() {
    localStorage.removeItem("envirovoice_gamertag");
    localStorage.removeItem("envirovoice_url");
  }
  ...
  updateGameStatus(isInGame) {
    ...
    if (isInGame) {
      this.elements.gameStatus.innerHTML =
        '<span style="color:#3c8527;">✓ Connected to Minecraft server</span>';
...
class VoiceChatApp {
  ...
  async init() {
    // Validate HTTPS (required for getUserMedia on mobile)
    this.checkHTTPS();
    ...
    if (saved) {
      ...
      // Pre-fill inputs in case user edits later
      this.ui.elements.gamertagInput.value = saved.gamertag;
      this.ui.elements.roomUrlInput.value = saved.url;
      // Show profile screen
  ...
  setupEventListeners() {
    ...
    this.ui.elements.saveProfileBtn.addEventListener("click", () => {
      ...
      if (!url) {
        alert("⚠️ Please enter a Voice Channel URL.");
        return;
      }

      // Save to localStorage
      UIManager.saveProfile(gamertag, url);
      this.currentGamertag = gamertag;
      this.ui.updateGamertagStatus(gamertag);
    ...
    // --- Screen 2: Edit button -> go back to Screen 1 ---
